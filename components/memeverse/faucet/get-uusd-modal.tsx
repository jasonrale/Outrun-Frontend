...
interface GetUUSDModalProps {
  isOpen: boolean
  onClose: () => void
}

export function GetUUSDModal({ isOpen, onClose }: GetUUSDModalProps) {
  const [fromAmount, setFromAmount] = useState("0.00")
  const [toAmount, setToAmount] = useState("0.00")
  const [selectedToken, setSelectedToken] = useState("sUSDS")
  const [isTokenDropdownOpen, setIsTokenDropdownOpen] = useState(false)

  // Mock token options
  const tokenOptions = [
    { symbol: "sUSDS", balance: "0" },
    { symbol: "aUSDC", balance: "0" },
    { symbol: "USDC", balance: "0" },
  ]

  const handleGetUUSD = () => {
    console.log(`Converting ${fromAmount} ${selectedToken} to UUSD...`)
        ...
        >
          <div className="flex items-center justify-between mb-1">
            ...
            <div className="flex items-center">
              <TokenIcon symbol="UUSD" size={20} className="mr-2" />
              <span className="text-green-300 font-medium">UUSD</span>
            </div>
          </div>
          <div className="flex items-center justify-between text-xs">
