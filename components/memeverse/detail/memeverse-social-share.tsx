...
export function MemeverseSocialShare({ isOpen, onClose, project, triggerSource }: MemeverseSocialShareProps) {
  ...
  const symbol = project.symbol // Declare the symbol variable
  ...
  if (triggerSource === "stake") {
    ...
    shareText = selectedTweets[randomTweetIndex].replace("{symbol}", symbol)
  }
  ...
  const getIntroMessage = () => {
    if (triggerSource === "genesis") {
      ...
      return `Congrats on joining the ${symbol} community! Boost community value, invite friends, and earn great referral rewards!`
    } else if (triggerSource === "claimPol") {
      return `Dear ${symbol} Genesis member, passionately promote the community, boost your returns, invite more friends, and share generous referral rewards!`
    } else if (triggerSource === "stake") {
      return `Congrats on staking ${symbol}! Keep boosting community value, invite more friends, and earn generous referral rewards!`
    } else if (triggerSource === "claimDAORewards") {
  ...
  return (
    <BaseModal isOpen={isOpen} onClose={onClose} showCloseButton={false} maxWidth="max-w-md">
